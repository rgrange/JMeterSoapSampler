...
public class XPathNSAwarePanel extends JPanel {
	...
	private static final Logger log = LoggerFactory.getLogger(XPathNSAwarePanel.class);

	private Document testDoc;
	private JCheckBox negated;
	private UndoableJTextField xpath;
	private JButton checkXPath;
	private TreeMap<String, String> namespaceMap = null;
	...
	private boolean validXPath(String xpathString, boolean showDialog) {
		...
		try {
			...
			if(this.namespaceResolver != null) {
				...
				String uri;
				String var13;
				for(Iterator<String> var12 = this.namespaceMap.keySet().iterator(); var12.hasNext(); this.namespaceResolver.setAttribute(var13, uri)) {
					var13 = var12.next();
					uri = this.namespaceMap.get(var13);
