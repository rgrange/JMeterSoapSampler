...
import com.jmeter.protocol.soap.sampler.MtomSOAPSampler;
...
public class MtomSOAPSamplerGui extends AbstractSamplerGui {

	private static final long serialVersionUID = 76805212533699529L;

	private static final Logger log = LoggerFactory.getLogger(MtomSOAPSamplerGui.class);
	...
	private JComboBox<String> attachmentContentType;
	private JComboBox<String> attachmentType;
	...
	private JComboBox<String> attachmentSelContentType;
	private JTextField attachmentSelContentId;
	private JRadioButton firstWithContentType;
	private JComboBox<String> firstWithSoapProtocolVersion;
	...
	public TestElement createTestElement() {
		MtomSOAPSampler sampler = new MtomSOAPSampler();
	...
	public void modifyTestElement(TestElement s) {
		...
		if(s instanceof MtomSOAPSampler) {
			MtomSOAPSampler sampler = (MtomSOAPSampler)s;
	...
	private void init() {
		...
		this.firstWithSoapProtocolVersion = new JComboBox<String>(soapProtocolVersions);
		...
		this.attachmentContentType = new JComboBox<String>(contentTypes);
		this.attachmentContentType.setEditable(true);
		this.attachmentType = new JComboBox<String>(attachmentTypes);
		...
		this.attachmentSelContentType = new JComboBox<String>(contentTypes);
		...
		typeCol.setCellEditor(new MtomSOAPSamplerGui.ComboBoxTableCellEditor(attachmentTypes, false));
		TableColumn attNameCol = tcm.getColumn(1);
		MtomSOAPSamplerGui.ConditionalFileBrowseCellEditor fbce = new MtomSOAPSamplerGui.ConditionalFileBrowseCellEditor(this, 30, SOAPMessages.getResString("soap_browse_attachment"), 0, SOAPAttachmentTableModel.TYPE_RESOURCE);
		attNameCol.setCellEditor(fbce);
		TableColumn contentTypeCol = tcm.getColumn(2);
		contentTypeCol.setCellEditor(new MtomSOAPSamplerGui.ComboBoxTableCellEditor(contentTypes, true));
		...
		this.attachButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				SOAPAttachmentTableModel satm = (SOAPAttachmentTableModel)MtomSOAPSamplerGui.this.attachmentTable.getModel();
				satm.add((String)MtomSOAPSamplerGui.this.attachmentType.getSelectedItem(), MtomSOAPSamplerGui.this.attachmentName.getText(), (String)MtomSOAPSamplerGui.this.attachmentContentType.getSelectedItem(), MtomSOAPSamplerGui.this.contentId.getText());
			}
		});
		this.removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				SOAPAttachmentTableModel satm = (SOAPAttachmentTableModel)MtomSOAPSamplerGui.this.attachmentTable.getModel();
				int row = MtomSOAPSamplerGui.this.attachmentTable.getSelectedRow();
				int[] selRows = MtomSOAPSamplerGui.this.attachmentTable.getSelectedRows();
				...
				if(selRow >= 0) {
					MtomSOAPSamplerGui.this.attachmentTable.setRowSelectionInterval(selRow, selRow);
				}
		...
		this.browseAttachmentButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				MtomSOAPSamplerGui.ChooseResult cr = MtomSOAPSamplerGui.this.chooseFile(MtomSOAPSamplerGui.this.attachmentName.getText());
				if(cr.chosen) {
					MtomSOAPSamplerGui.this.attachmentName.setText(cr.chosenValue);
					MtomSOAPSamplerGui.this.attachmentType.setSelectedItem(SOAPAttachmentTableModel.TYPE_RESOURCE);
				}
		...
		this.treatAttachmentAsResponseCbx.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				boolean enabled = MtomSOAPSamplerGui.this.treatAttachmentAsResponseCbx.isSelected();
				MtomSOAPSamplerGui.this.firstWithContentType.setEnabled(enabled);
				MtomSOAPSamplerGui.this.withContentId.setEnabled(enabled);
				if(enabled) {
					MtomSOAPSamplerGui.this.attachmentSelContentType.setEnabled(MtomSOAPSamplerGui.this.firstWithContentType.isSelected());
					MtomSOAPSamplerGui.this.attachmentSelContentId.setEnabled(MtomSOAPSamplerGui.this.withContentId.isSelected());
				} else {
					MtomSOAPSamplerGui.this.attachmentSelContentType.setEnabled(false);
					MtomSOAPSamplerGui.this.attachmentSelContentId.setEnabled(false);
				}
		...
		MtomSOAPSamplerGui.SelectionListener listener = new MtomSOAPSamplerGui.SelectionListener(this.attachmentTable, this);
	...
	public MtomSOAPSamplerGui.ChooseResult chooseFile(String originalContents) {
		MtomSOAPSamplerGui.ChooseResult cr = new MtomSOAPSamplerGui.ChooseResult();
		cr.chosen = false;
		...
		try {
			...
			if(!this.useRelativePaths.isSelected()) {
				...
				if(ioe.startsWith(baseCanonPath)) {
					...
					ArrayList<String> selFilePathComponents = new ArrayList<>();
					ArrayList<String> basePathComponents = new ArrayList<>();
					...
					if(lastMatchedIndex > 1) {
						...
						for(int selPathIndex = lastMatchedIndex + 1; selPathIndex < selFilePathComponents.size(); ++selPathIndex) {
							...
							var17 = false;
							var16.append(selFilePathComponents.get(selPathIndex));
						}
	...
	private static void splitFilePath(File f, ArrayList<String> al) throws IOException {
	...
	public void configure(TestElement el) {
		super.configure(el);
		MtomSOAPSampler sampler = (MtomSOAPSampler)el;
	...
	public static class SelectionListener implements ListSelectionListener {
		JTable table;
		MtomSOAPSamplerGui gui;

		SelectionListener(JTable table, MtomSOAPSamplerGui gui) {
			this.table = table;
			this.gui = gui;
		}
	...
	public static class ConditionalFileBrowseCellEditor extends AbstractCellEditor implements TableCellEditor {
		...
		private MtomSOAPSamplerGui gui;
		...
		public ConditionalFileBrowseCellEditor(MtomSOAPSamplerGui gui, int textFieldSize, String buttonTitle, int typeColumn, Object colValue) {
			this.gui = gui;
			...
			this.button.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					MtomSOAPSamplerGui.ChooseResult cr = ConditionalFileBrowseCellEditor.this.getGui().chooseFile(ConditionalFileBrowseCellEditor.this.textField.getText());
					if(cr.chosen) {
		...
		private MtomSOAPSamplerGui getGui() {
			return this.gui;
		}
	...
	public static class ComboBoxTableCellEditor extends AbstractCellEditor implements TableCellEditor {
		...
		public ComboBoxTableCellEditor(String[] values, boolean editable) {
			this.values = values;
			this.component = new JComboBox<String>(this.values);
